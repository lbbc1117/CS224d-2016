...
def normalizeRows(x):
    ...
    x = x**2
    x = np.sqrt( x / np.sum(x, axis=1, keepdims=True) )
    ### END YOUR CODE
...
def skipgram(currentWord, C, contextWords, tokens, inputVectors, outputVectors, 
    ...
    inputVec = inputVectors[current, :]
    
    for contextWord in contextWords:
        target = tokens[contextWord]

        curCost, curGradIn, curGradOut = \
            word2vecCostAndGradient(inputVec, target, outputVectors, dataset)
        
        cost += curCost
        gradIn[current, :] += curGradIn
        gradOut += curGradOut
...
def cbow(currentWord, C, contextWords, tokens, inputVectors, outputVectors, 
    ...
    inputVec = np.zeros(inputVectors.shape[1])
    
    for contextWord in contextWords:
        inputVec += inputVectors[tokens[contextWord], :]

    cost, curGradIn, gradOut = word2vecCostAndGradient(inputVec, target, outputVectors, dataset)

    for contextWord in contextWords:
        gradIn[tokens[contextWord], :] += curGradIn
