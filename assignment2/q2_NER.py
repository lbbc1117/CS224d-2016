...
class NERModel(LanguageModel):
  ...
  def add_embedding(self):
    ...
    with tf.device('/cpu:0'):
      ######### YOUR CODE HERE
      # Init L from scrach
      #L = tf.Variable(tf.random_uniform([len(self.wv), self.config.embed_size], -1.0, 1.0), name="L")
      # Init L with pre-trained word-vectors, which will speed up the training process
  ...
  def add_model(self, window):
    ...
    # Maybe not use dropout before softmax
    #output = tf.nn.dropout(output, self.dropout_placeholder)
    
    self.reg_loss = tf.nn.l2_loss(W) + tf.nn.l2_loss(U)
    ######### END YOUR CODE
    return output 
  ...
  def add_loss_op(self, pred):
    ...
    """
    ######### YOUR CODE HERE
    # Pred should be unnormalized classification scores
    sample_losses = tf.nn.softmax_cross_entropy_with_logits(pred, self.labels_placeholder)
...
def test_NER():
  ...
  with tf.Graph().as_default():
    ...
    with tf.Session() as session:
      ...
      # Codes below will trigger a runtime warning about averaging a empty list
      # This warning is caused by all-zero array model.y_test and should be ignored
      # print np.all(model.y_test==0)
